...
OWNER_ID = 5392468999
GOOGLE_API_KEY = os.environ.get('GOOGLE_API_KEY', "xxxxxxxxx")
...
if GEMINI_AVAILABLE and GOOGLE_API_KEY:
    ...
    AI_MODEL = None
    logger.warning("Gemini AI not available — missing library or API key")
...
def handle_ai_message(message):
    ...
    try:
        ...
        try:
            bot.send_message(
                user_id, header + formatted, parse_mode="Markdown"
            )
        except Exception:
...
def cb_browse_semester(call):
    ...
    state["browse_dept"] = dept
    state["browse_year"] = year
    state["browse_semester"] = semester
    set_state(user_id, state)
    remove_inline_keyboard(call.message.chat.id, call.message.message_id)
    ...
    loc_line = f"*{dept_display}* · {year} · {sem_label}" if year else f"*{fac_display}* · {sem_label}"
    if not books:
        bot.send_message(
            user_id,
            t(user_id, "no_books"),
...
def run_bot():
    logger.info("🤖 University Book Sharing Bot (mtu.ai) is running...")
