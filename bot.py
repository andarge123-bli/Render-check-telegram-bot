...
# These are tried one by one. If one fails for any reason, the next is tried.
_AI_MODELS = [
    "gemini-2.0-flash",
...
# ── Sticky model: the last model that successfully answered ──────────────────
# Once a model works it stays as the first choice for ALL future requests.
# Only swapped out when it actually fails — no unnecessary rotation.
_sticky_model = None
_sticky_model_lock = threading.Lock()


def _get_model_order():
    """Return _AI_MODELS with the sticky model moved to the front."""
...
# These errors mean the API key itself is bad → stop trying models, rotate key
_KEY_BAD_SIGNALS = (
...
# These are short network blips → worth a quick retry on the same model
_NETWORK_SIGNALS = (
...
class _KeyBadError(Exception):
    """Raised inside _try_models when the API key is definitively bad."""
...
def _safe_str(value):
    """Convert anything to a stripped string safely."""
    try:
        if value is None:
            return ""
...
def _extract_text(response):
    """
    Pull text from a Gemini response object safely.
    Returns None  → safety-blocked
    Returns ""    → empty / no text
    Returns str   → actual response text
    """
    try:
        if response.candidates:
            candidate = response.candidates[0]
            finish = _safe_str(getattr(candidate, "finish_reason", ""))
            if "SAFETY" in finish or finish == "2":
                return None  # safety blocked
            content = getattr(candidate, "content", None)
            if content:
                parts = getattr(content, "parts", None) or []
                text = "".join(
                    _safe_str(getattr(p, "text", ""))
                    for p in parts
...
def _build_contents(history, prompt):
    """Build Gemini Content list from stored history + current prompt."""
    contents = []
    for entry in history:
        try:
            role = _safe_str(entry.get("role", "user")) or "user"
            # Gemini only accepts "user" or "model" — map anything else to "user"
            if role not in ("user", "model"):
                role = "user"
            parts_data = entry.get("parts", [])
            if isinstance(parts_data, list):
                text_val = _safe_str(parts_data[0]) if parts_data else ""
            else:
                text_val = _safe_str(parts_data)
            if text_val:
                contents.append(genai_types.Content(
                    role=role,
                    parts=[genai_types.Part(text=text_val)],
                ))
        except Exception:
            continue  # skip any corrupt history entry
    if prompt:
        contents.append(genai_types.Content(
            role="user",
...
def _call_gemini(client, model, contents, with_system=True):
    """One Gemini API call. Returns text, "" (empty), or None (safety). Raises on error."""
    if with_system:
...
def _try_models(client, contents, label):
    """
    Try models in smart order: sticky model first, then the rest.
    - Returns text str   → success  (also updates sticky model)
    - Returns None       → every model was safety-blocked
    - Returns ""         → all models failed (non-key errors)
    - Raises _KeyBadError → this API key is bad, caller should rotate

    Sticky model behaviour:
      • The model that last worked is always tried first.
      • If that model fails, it is cleared and all other models are tried.
      • Whichever model succeeds becomes the new sticky model.
      • Result: after the first successful call, every future call goes
        straight to the proven model — zero unnecessary fallbacks.
    """
    any_real_failure = False

    # Sticky model is always at index 0 (if set)
    model_order = _get_model_order()

    for idx, model in enumerate(model_order):
        is_sticky = (idx == 0 and _sticky_model == model)
        model_failed = False   # did THIS model fail (not blocked)
        model_blocked = False  # did safety filter block this model

        for with_sys in (True, False):
            if model_blocked or model_failed:
                break
            for attempt in range(3):
                try:
                    result = _call_gemini(client, model, contents, with_sys)
                    if result is None:
                        logger.warning("[%s] model=%s safety-blocked", label, model)
                        model_blocked = True
                        break
                    if result:
                        # ✅ This model worked — lock it in as sticky
                        _set_sticky_model(model)
                        return result
                    # Empty — retry
                    logger.warning("[%s] model=%s empty (attempt %d/3)", label, model, attempt + 1)
                    time.sleep(1)
                except Exception as e:
                    err_str = str(e).lower()
                    logger.warning("[%s] model=%s sys=%s attempt=%d: %s",
                                   label, model, with_sys, attempt + 1, e)
                    any_real_failure = True
                    model_failed = True
                    if _is_key_bad(err_str):
                        # Key is bad — if this was the sticky model, clear it
                        if is_sticky:
                            _clear_sticky_model()
                        raise _KeyBadError(str(e))
                    if _is_network_err(err_str) and attempt < 2:
                        time.sleep(2 ** attempt)
                        model_failed = False  # network blip — give it another chance
                        continue
                    break

        # If the sticky model just failed, clear it so the next call
        # doesn't waste time on a broken model
        if is_sticky and (model_failed or model_blocked):
            _clear_sticky_model()

    if not any_real_failure:
        return None  # every model was safety-blocked
    return ""  # all failed due to real errors


def _nuclear_fallback(user_text):
    """
    Absolute last resort: simplest possible Gemini call.
    No history, no system prompt, no config — just a plain string.
    Tries every key × every model until something comes back.
    """
    for api_key in GOOGLE_API_KEYS:
        ...
        for model in _AI_MODELS:
            for attempt in range(3):
                try:
                    # Simplest SDK form: pass the message as a plain string
                    response = client.models.generate_content(
                        model=model,
                        contents=user_text,
                    )
                    txt = _safe_str(getattr(response, "text", ""))
                    if txt:
                        logger.info("Nuclear fallback succeeded: model=%s", model)
                        _set_sticky_model(model)  # remember this model for next time
                        return txt
                except Exception as e:
                    err_str = str(e).lower()
                    logger.warning("Nuclear model=%s attempt=%d: %s", model, attempt + 1, e)
                    if _is_key_bad(err_str):
...
def _ai_worker(user_id, user_text, lang, history, prompt, thinking_msg):
    """
    Runs the full AI attempt matrix in a background thread.

    Strategy (in order, never skips a step):
      Round 1 → Round 2 (5s delay) → Round 3 (10s delay)
        Per round: every key × [Pass1 +hist] → [Pass2 -hist] → [Pass3 bare]
          Per pass: every model × (with_system / without) × 3 network retries
      Nuclear fallback: plain-string call, all keys × all models
    """
    raw = ""
    succeeded = False
    ...
    for global_round in range(3):
        if global_round > 0:
            delay = global_round * 5  # 5s, then 10s
        ...
        for _ki in range(len(GOOGLE_API_KEYS)):
            ...
            # ── Pass 2: no history (may be causing the failure) ───────────────
            try:
                result = _try_models(
            ...
            # ── Pass 3: bare Content object — no system, no history ───────────
            try:
...
def handle_ai_message(message):
    """Entry point — validates input, shows thinking bubble, then hands off to background thread."""
    user_id = message.from_user.id
    lang = get_lang(user_id)

    # ── Guard: message.text can be None for photos/stickers/etc. ─────────────
    ...
    # ── Identity questions answered locally ───────────────────────────────────
    ...
    # ── Dispatch to background thread so other users aren't blocked ──────────
    threading.Thread(
        target=_ai_worker,
...
def run_bot():
    ...
    # On Render, the old instance and new instance overlap briefly during deploys.
    # 15s gives the old one time to receive SIGTERM and stop cleanly.
    ...
    while not _shutdown_event.is_set():
        ...
        # ── Stop any leftover polling session on this process ─────────────────
        try:
            bot.stop_polling()
        except Exception:
            pass

        # ── Clear webhook + drop stale updates ───────────────────────────────
        ...
        try:
            ...
            # infinity_polling swallows 409 errors internally and retries every 3s,
            # so our 90-second conflict delay never fires.
            # bot.polling(none_stop=False) raises exceptions directly to this loop.
            bot.polling(
                none_stop=False,
                timeout=60,
            ...
            # polling() returned normally (stop_polling() was called) — exit loop
            logger.info("Polling stopped cleanly.")
            break
            ...
            if "409" in err_str or "conflict" in err_str:
                # Another instance is still running — wait 90s for it to fully die
