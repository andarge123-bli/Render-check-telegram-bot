...
# Reads are served instantly from RAM.
# Writes update RAM immediately (so the user gets a fast response) and then
# upload to the Telegram channel in the background for persistence.

_db_cache = None          # in-memory copy of database.json
_states_cache = None      # in-memory copy of user_choices.json
_db_lock = threading.Lock()
_states_lock = threading.Lock()

# Single-threaded executors keep uploads sequential (no race conditions)
...
def _save_index():
    """Update the pinned index message in the channel with latest file IDs."""
    payload = json.dumps({
    ...
    try:
        ...
        try:
            ...
            logger.warning("Could not pin index (make sure bot is admin): %s", pin_err)
    except Exception as e:
        logger.error("Failed to save DB index: %s", e)


def _load_index():
    """On startup: read pinned message → populate DB_MSG_IDS → warm the cache."""
    global _db_cache, _states_cache
    try:
        ...
        if chat.pinned_message and chat.pinned_message.text:
            ...
            if text.startswith("MTU_BOT_INDEX:"):
                ...
                # Warm the in-memory cache so first requests are instant
...
def load_db() -> dict:
    ...
    with _db_lock:
        if _db_cache is None:
            _db_cache = {"books": [], "users": {}}
        return _db_cache


def save_db(data: dict):
    """Update cache immediately (fast), then upload to channel in background."""
    global _db_cache
    with _db_lock:
        _db_cache = data
...
def _bg_save_db(data: dict):
    """Background: upload database.json to channel."""
    if not DB_CHANNEL_ID:
        return
    try:
...
def _bg_save_states(states: dict):
    """Background: upload user_choices.json to channel."""
    if not DB_CHANNEL_ID:
        return
    try:
...
def handle_document(message):
    ...
    fac_display = strip_emoji(faculty)
    dept_display_s = strip_emoji(dept) if dept else fac_display
    sem_label_s = "Semester 1" if semester == "Sem1" else "Semester 2"
    loc_line = f"*{dept_display_s}*" + (f" · {year}" if year else "") + (f" · {sem_label_s}" if semester else "")
    next_prompt = (
        f"{t(user_id, 'upload_success')}\n\n"
        f"📎 *Send another file* to the same location:\n"
        f"📍 {loc_line}\n\n"
        f"_Or tap 🏠 Menu to go back._"
    )
    bot.send_message(
        user_id,
        next_prompt,
...
def run_bot():
    ...
    # Clear any existing webhook quickly before polling
    logger.info("Startup — clearing webhook...")
    ...
    logger.info("Starting polling (infinity mode)...")
