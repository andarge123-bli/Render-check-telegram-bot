...
def cmd_aicheck(message):
    ...
    bot.send_message(user_id, "⏳ Running live API test...")
    ...
    try:
        ...
        result = response.text or "(empty response)"
        bot.send_message(user_id, f"✅ Live test passed!\nResponse: {result[:200]}")
    except Exception as e:
        bot.send_message(user_id, f"❌ Live test FAILED!\nError:\n`{str(e)[:500]}`", parse_mode="Markdown")
    ...
    while True:
        ...
        try:
            ...
            if "409" in err_str or "Conflict" in err_str:
                logger.warning("Conflict detected (409) — another instance may be running. Waiting 30s before retry...")
                time.sleep(30)
            else:
