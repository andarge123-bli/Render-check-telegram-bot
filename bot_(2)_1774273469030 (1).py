...
def _try_models(client, contents, label):
    """
    Try every model in _AI_MODELS with the given contents.
    - Returns text str  → success
    ...
    """
    any_real_failure = False  # track whether we got a real error (not safety)
    ...
    for model in _AI_MODELS:
        ...
        for with_sys in (True, False):
            ...
            for attempt in range(3):
                try:
                    ...
                    if result is None:
                        # Safety filter — this model/prompt combination is blocked
                        logger.warning("[%s] model=%s safety-blocked", label, model)
                        model_blocked = True
                        break
                    if result:
                        return result  # success
                    # Empty response — retry after short wait
        ...
        # If model_blocked and no real failure yet, keep safety_blocked_all assumption
...
def run_bot():
    ...
    while not _shutdown_event.is_set():
        ...
        try:
            ...
            # IMPORTANT: use bot.polling(none_stop=False), NOT infinity_polling.
