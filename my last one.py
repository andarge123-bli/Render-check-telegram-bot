...
def run_bot():
    ...
    # Wait a moment to let any previous instance release the polling lock on Telegram
    time.sleep(10)
    while not _shutdown_event.is_set():
        # Always stop any leftover polling session first
        try:
            bot.stop_polling()
        except Exception:
            pass
        # Clear any webhook and drop stale pending updates
        ...
        try:
            ...
            if "409" in err_str or "Conflict" in err_str:
                logger.warning("Conflict (409) — waiting 90s before retry...")
                _shutdown_event.wait(timeout=90)
            elif "ConnectionError" in err_str or "ReadTimeout" in err_str or "ConnectTimeout" in err_str:
